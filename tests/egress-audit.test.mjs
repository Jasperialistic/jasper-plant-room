...
const [index, navigation, worker, workflow, uploadQueue, kemuri, bundledImages] = await Promise.all([
...
test('media patches install before the initial cloud load', () => {
  const patchScript = index.indexOf('v47-mobile-navigation.js?v=4.42.0');
  const uploadScript = index.indexOf('v48-upload-queue.js?v=4.42.0');
  const designScript = index.indexOf('v48-kemuri.js?v=4.42.0');
...
test('service worker and page use the same cache-busted media patch', () => {
  assert.match(worker, /jasper-plant-room-shell-v4\.42\.0/);
  assert.match(worker, /v47-mobile-navigation\.js\?v=4\.42\.0/);
  assert.match(worker, /v48-upload-queue\.js\?v=4\.42\.0/);
  assert.match(worker, /v48-kemuri\.js\?v=4\.42\.0/);
  assert.match(workflow, /v47-mobile-navigation\.js\?v=4\.42\.0/);
