...
`;
  ...
  function syncAddPlantZoneOptions(){
    ...
    let zones=[];
    try{
      if(typeof db!=='undefined'&&Array.isArray(db.locations)&&db.locations.length)zones=db.locations;
      else if(Array.isArray(window.PLANT_DATA?.locations))zones=window.PLANT_DATA.locations;
    }catch(_error){zones=[];}
    if(!zones.length){
      const filter=document.getElementById('locationFilter');
      zones=(filter?[...filter.options]:[])
        .map(option=>({name:String(option.value||'').trim()}))
        .filter(zone=>zone.name);
    }
    if(!zones.length){
      zones=[...document.querySelectorAll('#locationCards [data-zone-name]')]
        .map(card=>({name:String(card.dataset.zoneName||'').trim()}))
        .filter(zone=>zone.name);
    }
    const names=[...new Set(zones.map(zone=>String(zone?.name||'').trim()).filter(Boolean))];
    const current=select.value;
    const signature=names.join('|');
    const rendered=[...select.options]
      .filter(option=>option.value&&option.value!=='__new__')
      .map(option=>option.value)
      .join('|');
    const hasCreate=[...select.options].some(option=>option.value==='__new__');
    if(rendered!==signature||!hasCreate){
      const placeholder=document.createElement('option');
      placeholder.value='';placeholder.textContent='Choose a growing zone';
      const create=document.createElement('option');
      create.value='__new__';create.textContent='+ Create new growing zone…';
      ...
      if(current==='__new__'||names.includes(current))select.value=current;
    }
    if(select.dataset.v4381ZoneBound!=='1'){
      select.dataset.v4381ZoneBound='1';
      select.addEventListener('change',()=>{
        if(select.value!=='__new__')return;
        requestAnimationFrame(()=>document.getElementById('newPlantNewZone')?.focus({preventScroll:true}));
      });
    }
    if(typeof toggleNewZoneField==='function')toggleNewZoneField();
  }
...
/* Plant profile: Gallery / Growth / Details / Care / Care log sections. */
(function(){
  const mq=window.matchMedia('(max-width:700px)');
  const css=`
@media(max-width:700px){
  ...
  #plantDialog .v411-tab{
    ...
    background:transparent;color:#83978e;font:780 10.5px/1 system-ui,sans-serif;letter-spacing:.005em;white-space:nowrap;
...
@media(max-width:370px){#plantDialog .v411-tab{font-size:9.5px;padding-inline:1px}}
@media(min-width:701px){
  #plantDialog .v411-tabs{display:none!important}#plantDialog .v411-panel{display:block!important;min-height:0!important;padding:0!important}
  #plantDialog .profile-tab[data-profile-tab="history"]{border-color:rgba(102,145,124,.34)}
  #plantDialog .profile-panel[data-panel="history"]{margin-top:2px}
}
.v438-log-paper{
  position:relative;margin:8px 0 4px;padding:4px 15px 4px 38px;overflow:hidden;
  border:1px solid rgba(107,147,128,.3);border-radius:16px;
  background:
    linear-gradient(90deg,transparent 26px,rgba(187,103,97,.2) 26px,rgba(187,103,97,.2) 27px,transparent 27px),
    repeating-linear-gradient(180deg,rgba(18,34,28,.98) 0,rgba(18,34,28,.98) 31px,rgba(106,143,125,.095) 32px);
  box-shadow:inset 0 1px 0 rgba(255,255,255,.03),0 9px 24px rgba(0,0,0,.2)
}
.v438-log-summary{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px 0 9px;color:#81988d;font:700 9px/1.25 ui-monospace,SFMono-Regular,Consolas,monospace;letter-spacing:.08em;text-transform:uppercase}
.v438-log-summary strong{color:#d5be85;font-size:10px}
.v438-log-entry{position:relative;padding:11px 0 12px;border-top:1px dashed rgba(113,151,132,.2)}
.v438-log-entry::before{content:'';position:absolute;left:-17px;top:18px;width:6px;height:6px;border:1px solid #628473;border-radius:50%;background:#14251f}
.v438-log-entry[data-kind="water"]::before{border-color:#8fc8ad;background:#396b58;box-shadow:0 0 8px rgba(117,190,155,.24)}
.v438-log-entry[data-kind="moist"]::before{border-color:#c6a75f;background:#6c5729}
.v438-log-head{display:flex;align-items:baseline;gap:9px;min-width:0}
.v438-log-action{min-width:0;color:#e6efea;font:750 12.5px/1.35 ui-monospace,SFMono-Regular,Consolas,monospace;overflow-wrap:anywhere}
.v438-log-entry[data-kind="water"] .v438-log-action{color:#b7dfca}
.v438-log-entry[data-kind="moist"] .v438-log-action{color:#dfc783}
.v438-log-date{flex:0 0 auto;margin-left:auto;color:#879d92;font:650 9.5px/1.25 ui-monospace,SFMono-Regular,Consolas,monospace;white-space:nowrap}
.v438-log-count{color:#71877d;font-size:9px;white-space:nowrap}
.v438-log-detail{margin:5px 0 0;color:#aebfb7;font:500 11px/1.5 ui-monospace,SFMono-Regular,Consolas,monospace;white-space:pre-wrap}
.v438-water-gap{margin:5px 0 0;color:#789b8a;font:650 9.5px/1.35 ui-monospace,SFMono-Regular,Consolas,monospace}
.v438-log-empty{padding:24px 0;color:#83988e;font:500 11px/1.6 ui-monospace,SFMono-Regular,Consolas,monospace;text-align:center}
@media(max-width:700px){
  #plantDialog .v411-panel[data-v411-panel="history"]{padding-top:10px!important}
  .v438-log-paper{margin-top:7px;padding-right:11px;padding-left:34px}
  .v438-log-head{gap:6px}.v438-log-action{font-size:11.5px}.v438-log-date{font-size:9px}
}
`;
  ...
  function activate(inner,name,{scroll=true}={}){
    if(!['gallery','growth','details','care','history'].includes(name))name='gallery';
  ...
  const v438WaterAction=action=>/^(?:watered(?: today)?|full shower\s*\/\s*water|dunked|reservoir topped(?: up)?|reservoir changed|.*\bflushed?\b)$/i.test(String(action||'').trim());
  const v438Kind=action=>v438WaterAction(action)?'water':/moist|checked/i.test(String(action||''))?'moist':'other';
  function v438Entries(inner){
    const name=String(inner.querySelector('.modal-head h2')?.textContent||'').trim().toLowerCase();
    if(!name||!Array.isArray(db?.careLog))return[];
    return db.careLog.filter(entry=>String(entry?.plant??entry?.plantName??entry?.plant_name??'').trim().toLowerCase()===name)
      .sort((a,b)=>String(b?.date??b?.careDate??b?.care_date??'').localeCompare(String(a?.date??a?.careDate??a?.care_date??'')));
  }
  function v438Grouped(rows){
    const groups=new Map();
    rows.forEach(entry=>{
      const date=entry?.date??entry?.careDate??entry?.care_date??'';
      const action=entry?.action??'Care entry',details=entry?.details??'';
      const key=JSON.stringify([date,action,details]);
      const group=groups.get(key);
      if(group)group.count+=1;else groups.set(key,{date,action,details,count:1});
    });
    return[...groups.values()];
  }
  function v438DateDays(newer,older){
    const a=Date.parse(String(newer||'').slice(0,10)+'T00:00:00Z');
    const b=Date.parse(String(older||'').slice(0,10)+'T00:00:00Z');
    return Number.isFinite(a)&&Number.isFinite(b)?Math.max(0,Math.round((a-b)/86400000)):0;
  }
  function renderV438History(panel,inner,button){
    if(!panel)return;
    const rows=v438Entries(inner),grouped=v438Grouped(rows);
    if(button){
      let chip=button.querySelector('.chip');
      if(!chip){button.append(' ');chip=document.createElement('span');chip.className='chip';button.appendChild(chip);}
      chip.textContent=String(rows.length);
    }
    let paper=panel.querySelector(':scope > .v438-log-paper');
    if(!paper){paper=document.createElement('div');paper.className='v438-log-paper';panel.appendChild(paper);}
    const key=JSON.stringify(rows.map(entry=>[entry?.id,entry?.date,entry?.action,entry?.details]));
    if(paper.dataset.v438Key===key)return;
    paper.dataset.v438Key=key;paper.replaceChildren();
    const summary=document.createElement('div');summary.className='v438-log-summary';
    const label=document.createElement('span');label.textContent='Care history';
    const total=document.createElement('strong');total.textContent=rows.length+' entr'+(rows.length===1?'y':'ies');
    summary.append(label,total);paper.appendChild(summary);
    if(!rows.length){
      const empty=document.createElement('div');empty.className='v438-log-empty';empty.textContent='No care entries recorded for this plant yet.';
      paper.appendChild(empty);return;
    }
    const waterDates=[...new Set(rows.filter(entry=>v438WaterAction(entry?.action)).map(entry=>entry?.date??entry?.careDate??entry?.care_date??'').filter(Boolean))].sort((a,b)=>String(b).localeCompare(String(a)));
    grouped.forEach(group=>{
      const item=document.createElement('article');item.className='v438-log-entry';item.dataset.kind=v438Kind(group.action);
      const head=document.createElement('div');head.className='v438-log-head';
      const action=document.createElement('strong');action.className='v438-log-action';action.textContent=group.action;
      head.appendChild(action);
      if(group.count>1){const count=document.createElement('span');count.className='v438-log-count';count.textContent='×'+group.count;head.appendChild(count);}
      const date=document.createElement('time');date.className='v438-log-date';date.dateTime=group.date;date.textContent=group.date?fmt(group.date):'No date';
      head.appendChild(date);item.appendChild(head);
      if(group.details){const detail=document.createElement('p');detail.className='v438-log-detail';detail.textContent=group.details;item.appendChild(detail);}
      if(v438WaterAction(group.action)){
        const index=waterDates.indexOf(group.date),older=index>=0?waterDates[index+1]:null;
        if(older){const days=v438DateDays(group.date,older),gap=document.createElement('p');gap.className='v438-water-gap';gap.textContent=days+' day'+(days===1?'':'s')+' since previous watering';item.appendChild(gap);}
      }
      paper.appendChild(item);
    });
  }
  function ensureDesktopHistory(inner){
    if(mq.matches)return;
    const tabs=inner.querySelector('.profile-tabs'),growth=inner.querySelector('.profile-panel[data-panel="growth"]');
    if(!tabs||!growth)return;
    let button=tabs.querySelector('[data-profile-tab="history"]');
    if(!button){
      button=document.createElement('button');button.type='button';button.className='profile-tab';button.dataset.profileTab='history';button.textContent='Care log';
      tabs.appendChild(button);
      button.addEventListener('click',()=>{
        tabs.querySelectorAll('[data-profile-tab]').forEach(tab=>tab.classList.toggle('active',tab===button));
        inner.querySelectorAll('.profile-panel[data-panel]').forEach(panel=>panel.classList.toggle('active',panel.dataset.panel==='history'));
      });
    }
    let panel=inner.querySelector('.profile-panel[data-panel="history"]');
    if(!panel){panel=document.createElement('div');panel.className='profile-panel';panel.dataset.panel='history';growth.insertAdjacentElement('afterend',panel);}
    renderV438History(panel,inner,button);
  }
  function moveContent(inner){
    ...
    const history=inner.querySelector('[data-v411-panel="history"]');
    if(!gallery||!growth||!details||!care||!history)return;
    ...
    if(!history.querySelector('.v411-section-title'))sectionIntro(history,'Care log','Newest entries first, with the gap between watering dates.');
    renderV438History(history,inner);
  ...
  function sync(){
    queued=false;if(!dlg.open)return;
    const inner=dlg.querySelector('.dialog-inner');if(!inner)return;
    if(!mq.matches){ensureDesktopHistory(inner);return;}
    if(!build(inner))return;
    moveContent(inner);
  }
...
.v437-last-watered{display:flex;align-items:center;justify-content:space-between;gap:14px;min-width:0;margin:8px 0 4px;padding:10px 12px;border:1px solid #3d6658;border-radius:13px;background:#173027;box-shadow:inset 0 1px 0 rgba(255,255,255,.025)}
.v437-last-watered-copy{min-width:0}.v437-last-watered span{display:block;color:#8db7a4;font-size:9px;font-weight:850;letter-spacing:.1em;text-transform:uppercase}
.v437-last-watered small{display:block;margin-top:3px;color:#7f9b8e;font-size:10px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.v437-last-watered strong{flex:0 0 auto;color:#b9e2ce;font-size:13px;line-height:1.25;text-align:right}
.v437-last-watered[data-empty="1"]{border-color:#2d463b;background:#12211c}.v437-last-watered[data-empty="1"] strong{color:#91a49b}
...
@media(max-width:700px){
  ...
  .v437-last-watered{margin-top:10px;padding:10px 11px}
  .v437-last-watered strong{font-size:12px}
}
`;
  ...
  const wateringAction=action=>/^(?:watered(?: today)?|full shower\s*\/\s*water|dunked|reservoir topped(?: up)?|reservoir changed|.*\bflushed?\b)$/i.test(String(action||'').trim());
  const entryAction=entry=>entry?.action??entry?.lastAction??entry?.last_action??'';
  const entryDate=entry=>entry?.careDate??entry?.care_date??entry?.date??entry?.createdAt??entry?.created_at??'';
  const entryTime=entry=>Date.parse(entry?.createdAt??entry?.created_at??entryDate(entry))||0;
  function entryMatchesPlant(entry,p){
    const entryId=entry?.plantId??entry?.plant_id??entry?.cloudId??entry?.cloud_id;
    const plantIds=[p?.cloudId,p?.cloud_id,p?.id].filter(value=>value!==null&&value!==undefined&&value!=='').map(String);
    if(entryId!==null&&entryId!==undefined&&entryId!=='')return plantIds.includes(String(entryId));
    const entryName=String(entry?.plant??entry?.plantName??entry?.plant_name??'').trim().toLowerCase();
    const plantName=String(p?.name??'').trim().toLowerCase();
    return !!entryName&&entryName===plantName;
  }
  function lastWateringEntry(p){
    const rows=Array.isArray(db?.careLog)?db.careLog:[];
    return rows.filter(entry=>entryMatchesPlant(entry,p)&&wateringAction(entryAction(entry))).sort((a,b)=>entryTime(b)-entryTime(a))[0]||null;
  }
  function renderLastWatered(actions,p){
    let card=document.querySelector('#plantDialog .v437-last-watered');
    if(!card){card=document.createElement('div');card.className='v437-last-watered';actions.before(card);}
    const entry=lastWateringEntry(p),date=entryDate(entry),action=entryAction(entry);
    card.dataset.empty=entry?'0':'1';
    card.innerHTML=entry
      ?`<div class="v437-last-watered-copy"><span>Last watered</span><small>${esc(action)}</small></div><strong>${esc(dateLabel(date))}</strong>`
      :'<div class="v437-last-watered-copy"><span>Last watered</span><small>No watering entry yet</small></div><strong>Not recorded</strong>';
  }
...
(function(){
  ...
  const dlg=document.getElementById('plantDialog');
  if(!dlg)return;
  const names=['gallery','growth','details','care','history'];
...
`;
  ...
  function restore(inner){
    ...
    if(actions?.classList.contains('v428-care-actions')){
      ...
      actions.querySelectorAll('[data-quick][data-v428-label]').forEach(button=>{
        const label=labels[button.dataset.quick];if(label&&button.textContent!==label)button.textContent=label;
        delete button.dataset.v428Label;button.removeAttribute('title');
      });
    }
    const title=inner.querySelector('.v428-details-title');
    const edit=inner.querySelector('#editPlantBtn');
    const editWasMobile=!!(edit&&(edit.classList.contains('v428-edit-details')||edit.dataset.v428Label==='edit'));
    const editWasInMobileTitle=!!(editWasMobile&&title&&edit.parentElement===title);
    if(title)title.classList.remove('v428-details-title');
    if(editWasMobile){
      edit.classList.remove('v428-edit-details');
      if(edit.textContent!=='Edit details')edit.textContent='Edit details';
      delete edit.dataset.v428Label;edit.removeAttribute('aria-label');edit.removeAttribute('title');
      if(editWasInMobileTitle&&actions)actions.appendChild(edit);
    }
  }
...
@media(min-width:701px){
  ...
  #photoLightboxStage .v432-photo-slide,#growthViewStage .v432-photo-slide{display:none!important}
  #photoLightboxStage .v432-photo-current,#growthViewStage .v432-photo-current{
    display:grid!important;width:100%!important;height:100%!important;place-items:center!important
  }
}
`;
  document.head.appendChild(style);
  ...
  function enhance(dlg){
    if(!mq.matches||!dlg||dlg.dataset.v432Carousel==='1')return;
    const stage=dlg.querySelector('#photoLightboxStage,#growthViewStage');
    const img=dlg.querySelector('#photoLightboxImg,#growthViewImg');
    if(!stage||!img)return;
  ...
  if(typeof mq.addEventListener==='function')mq.addEventListener('change',()=>{
    if(mq.matches){scan();return;}
    document.querySelectorAll('.v432-photo-rail').forEach(rail=>{
      rail.classList.remove('v432-dragging','v432-snapping');rail.style.removeProperty('--v432-drag-x');
    });
  });
})();
...
(function(){
  const style=document.createElement('style');
  style.id='v433SubtlePhotoControls';
  style.textContent=`
@media(max-width:700px){
  #photoLightboxPrev,#photoLightboxNext,#growthViewPrev,#growthViewNext{
    width:44px!important;height:60px!important;padding:0!important;border:0!important;
    border-radius:999px!important;background:transparent!important;color:transparent!important;
    font-size:0!important;box-shadow:none!important;opacity:.34;
    transition:opacity 160ms ease,background-color 160ms ease,transform 110ms ease;
    -webkit-tap-highlight-color:transparent
  }
  #photoLightboxPrev::before,#growthViewPrev::before,
  #photoLightboxNext::before,#growthViewNext::before{
    display:block;color:rgba(244,247,245,.82);font:300 29px/1 system-ui,sans-serif;
    text-shadow:0 2px 10px rgba(0,0,0,.92)
  }
  #photoLightboxPrev::before,#growthViewPrev::before{content:"‹"}
  #photoLightboxNext::before,#growthViewNext::before{content:"›"}
  #photoLightboxPrev:active,#photoLightboxNext:active,#growthViewPrev:active,#growthViewNext:active{
    opacity:.9;background:rgba(255,255,255,.075)!important;transform:translateY(-50%) scale(.91)
  }
  #photoLightboxPrev:focus-visible,#photoLightboxNext:focus-visible,
  #growthViewPrev:focus-visible,#growthViewNext:focus-visible{
    opacity:.9;outline:1px solid rgba(213,190,133,.55);outline-offset:-5px
  }
  #photoLightboxStage .v432-photo-rail.v432-dragging~.photo-lightbox-nav,
  #growthViewStage .v432-photo-rail.v432-dragging~.growth-view-nav{opacity:.06}

  #photoLightboxClose,#growthViewClose,.v492-photo-back{
    position:relative;width:44px!important;height:44px!important;padding:0!important;
    ...
    box-shadow:none!important;opacity:.5;
    transition:opacity 160ms ease,transform 110ms ease;
    -webkit-tap-highlight-color:transparent
  }
  #photoLightboxClose::before,#growthViewClose::before{
    content:"×";display:grid;place-items:center;width:29px;height:29px;border-radius:50%;
    border:1px solid rgba(255,255,255,.105);background:rgba(5,10,8,.28);
    color:rgba(243,247,245,.7);font:300 20px/1 system-ui,sans-serif;
    box-shadow:0 5px 16px rgba(0,0,0,.16);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px)
  }
  .v492-photo-back::before{
    content:"‹";display:grid;place-items:center;width:29px;height:29px;border-radius:50%;
    border:1px solid rgba(255,255,255,.085);background:rgba(5,10,8,.22);
    color:rgba(243,247,245,.62);font:300 24px/1 system-ui,sans-serif;
    box-shadow:0 5px 16px rgba(0,0,0,.13);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px)
  }
  #photoLightboxClose:active,#growthViewClose:active,.v492-photo-back:active{opacity:.95;transform:scale(.9)}
  #photoLightboxClose:focus-visible,#growthViewClose:focus-visible,.v492-photo-back:focus-visible{
    opacity:.95;outline:1px solid rgba(213,190,133,.55);outline-offset:-5px
  }
}
@media(max-width:700px) and (prefers-reduced-motion:reduce){
  #photoLightboxPrev,#photoLightboxNext,#growthViewPrev,#growthViewNext,
  #photoLightboxClose,#growthViewClose,.v492-photo-back{transition:none!important}
}
`;
  document.head.appendChild(style);
})();
